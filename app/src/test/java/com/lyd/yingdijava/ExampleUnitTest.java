...
public class ExampleUnitTest {
    ...
    public void myTest(){
        ...
        String smallString = "{\\\\\\\"普通\\\\\\\":7,\\\\\\\"稀有\\\\\\\":16,\\\\\\\"传说\\\\\\\":3,\\\\\\\"史诗\\\\\\\":4}";
        try {
...
//            }
            StringBuffer small = new StringBuffer(smallString.replaceAll("\\{","").replaceAll("}","").replaceAll("\\\\",""));
            String[] tempList = small.toString().split(",");
            HashMap<String,String> map = new HashMap<>();
            for (int i = 0; i < tempList.length; i++) {
//                System.out.println(tempList[i]);
                String[] tempS = tempList[i].replaceAll("\"","").split(":");
